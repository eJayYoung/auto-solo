...
import { createNextRound } from "@/lib/services/solo-workflow";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const round = await createNextRound(input.roundId);
    return NextResponse.json({ ok: true, data: round });
  } catch (error) {
    const message = error instanceof Error ? error.message : "生成下一轮失败";
