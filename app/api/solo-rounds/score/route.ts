...
import { scoreSoloRound } from "@/lib/services/solo-workflow";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const round = await scoreSoloRound(input.roundId);
    return NextResponse.json({ ok: true, data: round });
  } catch (error) {
    const message = error instanceof Error ? error.message : "一键评分失败";
